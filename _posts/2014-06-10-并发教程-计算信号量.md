...
Semaphore是一个同步器，允许对资源进行绑定。Semaphore、 CountDownLatch, CyclicBarrier 和 Exchanger等等都是在Java5中新引入的类。Semaphore 维护指定数量的访问许可。为了访问共享的资源，当前线程必须获取一个许可，如果`许可`被其他的线程耗尽，你就只能等待，直到`许可`时可用的。`许可`可由其他的线程释放。
...
只有一个许可的Semaphore作为一个二进制信号量，因为它只有2中状态：可用和不可用。二进制信号量可以实现互斥和关键部分只允许一个线程执行。线程将在acquire()处等待直到在调用Semaphore的release()方法临界处释放许可。
