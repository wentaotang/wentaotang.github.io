...
请注意Django默认的管理网站中的任何模板都是可覆盖的。要覆盖一个模板，只需要像刚才处理 base_site.html一样 – 从默认的目录下复制到你的自定义目录下，并修改它。

TEMPLATE_DIRS是一个迭代检查并加载Djangotemplates的文件系统目录，这个就是搜素路径。

现在我们在你的项目跟路径下面创建一个templates/admin的文件夹，现在从Django源代码中自带的默认Django管理模板的目录(django/contrib/admin/templates) 下复制 admin/base_site.html模板到你正在使用的TEMPLATE_DIRS 中任何目录的子目录 admin下。例如：如果你的 TEMPLATE_DIRS 中包含 '/mysite/templates' 目录， 如上所述，复制 django/contrib/admin/templates/admin/base_site.html 模板到 /mysite/templates/admin/base_site.html 。不要忘了是 admin 子目录

然后编辑文件并且替换通用的Django的文本为你认为合适额网站名。

模板包括了许多类似{% block branding %} 和{{title}} 这样的文本，这个{% 和 {{ 都是Django的模板语言，当Django渲染admin/base_site.html时，模板语言将评估并最终生成html页面。不要担心如果你现在不理解这些模板语言，我们将在第三部分深入学习模板语言。

请注意 Django 默认的管理网站中的任何模板都是可覆盖的。要覆盖一个模板，只需要像刚才处理base_site.html 一样 – 从默认的目录下复制到你的自定义目录下，并修改它。
