...
在这个表单中，"Poll"字段下拉框的内容是数据库中poll表的所有记录，jango 知道 ForeignKey 在管理网站中以 <select> 框显示。在本例中，选择框中仅存在一个 poll 。
...
在你的工程目录创建一个templates目录，模板可以放在你的文件系统的任何地方，Diango 都能访问((Django 能以任何用户身份在你的服务器上运行),然而，在你的项目中保存模板是一个好习惯。
...
TEMPLATE_DIRS是一个迭代检查并加载Django templates的文件系统目录，这个就是搜素路径。

现在我们在你的项目跟路径下面创建一个templates/admin的文件夹，现在从 Django 源代码中自带的默认 Django 管理模板的目录 (django/contrib/admin/templates) 下复制 admin/base_site.html 模板到你正在使用的 TEMPLATE_DIRS 中任何目录的子目录 admin 下。例如：如果你的 TEMPLATE_DIRS 中包含 '/mysite/templates' 目录， 如上所述，复制 django/contrib/admin/templates/admin/base_site.html 模板到 /mysite/templates/admin/base_site.html 。不要忘了是 admin 子目录。

然后编辑文件并且替换通用的Django的文本为你认为合适额网站名。

模板包括了许多类似{% block branding %} 和{{title}} 这样的文本，这个{% 和 {{ 都是Django的模板语言，当Django渲染admin/base_site.html时，模板语言将评估并最终生成html页面。不要担心如果你现在不理解这些模板语言，我们将在第三部分深入学习模板语言。
